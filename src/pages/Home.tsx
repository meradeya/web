    ...
    <div className="main-content container">
      <section className="hero">
        ...
        >
          ...
          <p className="hero-subtitle">
            Meradeya is the premier marketplace for exceptional items. Trade with confidence in our curated ecosystem.
          </p>
      ...
      <section>
        ...
        {isLoading && (
          <div className="grid">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="card skeleton" style={{ height: "400px" }}></div>
            ))}
          </div>
        )}
                  ...
                  <Link to={`/listing/${listing.id}`} className="card">
                    <div className="card-image-wrap">
                      {listing.firstPhotoUrl ? (
                         <img src={listing.firstPhotoUrl} alt={listing.title} className="card-image" />
                      ) : (
                        <div className="flex items-center justify-center" style={{ width: '100%', height: '100%', color: 'var(--border)' }}>
                           <ImageOff size={48} strokeWidth={1} />
                        </div>
                      )}
                      <span className="card-badge">{listing.condition.replace(/_/g, ' ')}</span>
                    </div>
