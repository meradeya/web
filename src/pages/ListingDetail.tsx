...
export function ListingDetail() {
  ...
  return (
    <div className="main-content container" style={{ paddingTop: '120px' }}>
      <Link to="/" className="btn btn-secondary" style={{ marginBottom: 40, alignSelf: 'flex-start' }}>
      ...
      >
        <div className="detail-gallery">
          {mainImage ? (
            ...
            <div className="text-muted flex items-center justify-center" style={{ width: '100%', height: '100%' }}>
        ...
        <div className="detail-info">
          <div className="detail-category">{listing.condition.replace(/_/g, ' ')}</div>
          <h1 className="detail-title">{listing.title}</h1>
          <div className="detail-price">{formatPrice(listing.price, listing.currency)}</div>
          
          <div className="flex gap-4 items-center" style={{ padding: '16px 0', borderBottom: '1px solid var(--border)', borderTop: '1px solid var(--border)' }}>
            <button className="btn btn-primary" style={{ flex: 1 }}>Purchase Item</button>
          ...
          <div className="meta-grid">
            <div className="meta-item">
              <span className="meta-label flex items-center gap-2"><MapPin size={14} /> Location</span>
              <span className="meta-value">{listing.location || 'Not specified'}</span>
            </div>
            <div className="meta-item">
              <span className="meta-label flex items-center gap-2"><Clock size={14} /> Listed</span>
              <span className="meta-value">{new Date(listing.createdAt).toLocaleDateString()}</span>
            </div>
            <div className="meta-item">
              <span className="meta-label flex items-center gap-2"><ShieldCheck size={14} /> Seller ID</span>
              <span className="meta-value" style={{ fontSize: '0.875rem', overflow: 'hidden', textOverflow: 'ellipsis' }}>{listing.sellerId}</span>
            </div>
            <div className="meta-item">
              <span className="meta-label">Status</span>
              <span className="meta-value" style={{ color: 'var(--accent)' }}>{listing.status}</span>
            </div>
          </div>
        </div>
      </motion.div>
